    ...
    def forward(self, hidden_states):
        ...
        if getattr(self, "adpt_sign", 0) == 1:
            ...
            if adpt_w1 is not None and adpt_w2 is not None:
                ...
                adapter_gate = torch.matmul(adapter_source, adpt_w1.T)
                adapter_up = torch.matmul(adapter_source, adpt_w2.T)
                adapter_out = self.down_proj(self.act_fn(adapter_gate) * adapter_up)
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        for decoder_layer in self.layers:
            ...
            hidden_states = decoder_layer(
                hidden_states,
                attention_mask=causal_mask_mapping[self.config.layer_types[layer]],
                position_embeddings=position_embeddings,
                position_ids=text_position_ids,
                past_key_values=past_key_values,
                use_cache=use_cache,
