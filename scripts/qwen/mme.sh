...
# Usage:
#   bash scripts/qwen/mme.sh
#   bash scripts/qwen/mme.sh memvr
#   bash scripts/qwen/mme.sh memvr 4
#   bash scripts/qwen/mme.sh memvr 6 "1,2,3,4,5,6"
MODE="${1:-memvr}"
NUM_GPUS="${2:-1}"
GPU_IDS_CSV="${3:-}"
if [[ "$MODE" != "memvr" && "$MODE" != "none" ]]; then
    echo "Invalid mode: $MODE"
    echo "Usage: bash scripts/qwen/mme.sh [memvr|none] [num_gpus] [gpu_ids_csv]"
    exit 1
fi
