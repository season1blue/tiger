...
entropy_threshold_default="0.35"    # Entropy trigger threshold (used when explicit target layers are not set).
starting_layer_default="2"          # Start layer index for entropy-based trigger checks.
...
use_state_drift_trigger_default="0" # 1 enables state-drift trigger; 0 falls back to entropy trigger.
