...
```
  ...
  ### Qwen2.5-VL on POPE (paper-to-code mapping)

  The Qwen2.5-VL integration follows the same VCD steps above:

  1. **Enable VCD sampling patch**
    - `experiments/eval/object_hallucination_vqa_qwen2_5_vl.py`
    - `from vcd_utils.vcd_sample import evolve_vcd_sampling` then `evolve_vcd_sampling()`

  2. **Construct distorted visual input**
    - `experiments/eval/object_hallucination_vqa_qwen2_5_vl.py`
    - `images_cd = add_diffusion_noise(inputs["pixel_values"], args.noise_step)`

  3. **Run contrastive decoding with $(\alpha, \beta)$**
    - `experiments/eval/object_hallucination_vqa_qwen2_5_vl.py`
    - `model.generate(..., images_cd=images_cd, cd_alpha=args.cd_alpha, cd_beta=args.cd_beta, ...)`

  4. **Model hook for the second visual branch**
    - `experiments/qwen2_5_vl/modeling_qwen2_5_vl.py`
    - `prepare_inputs_for_generation_cd(...)` routes `images_cd` to `pixel_values`

  5. **Where the VCD formula is actually applied**
    - `vcd_utils/vcd_sample.py`
    - clean/distorted logits are fused in sampling via contrastive logits.

  Run script:
  ```bash
  cd experiments
  bash cd_scripts/qwen2_5_pope.bash 55 coco random /path/to/Qwen2.5-VL-7B-Instruct 1 0.1 500
  ```
  Optional baseline (disable contrastive branch):
  ```bash
  cd experiments
  USE_CD=0 bash cd_scripts/qwen2_5_pope.bash 55 coco random /path/to/Qwen2.5-VL-7B-Instruct
  ```
