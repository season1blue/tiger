...
class GenerationMixin(ContinuousMixin):
    ...
    def _extract_generation_mode_kwargs(
        ...
        generation_mode_kwargs = {
            ...
            "assistant_model": assistant_model,
            "streamer": streamer,
            "opera_decoding": kwargs.pop("opera_decoding", None),
            "key_position": kwargs.pop("key_position", None),
            "scale_factor": kwargs.pop("scale_factor", 50.0),
            "threshold": kwargs.pop("threshold", 15),
            "num_attn_candidates": kwargs.pop("num_attn_candidates", 5),
            "penalty_weights": kwargs.pop("penalty_weights", 1.0),
        }
    ...
    def generate(
        ...
        opera_decoding: bool | None = None,
        key_position: dict | None = None,
        ...
        **kwargs,
        ...
        if opera_decoding is not None:
            kwargs["opera_decoding"] = opera_decoding
        if key_position is not None:
            kwargs["key_position"] = key_position
        kwargs["scale_factor"] = scale_factor
        kwargs["threshold"] = threshold
        kwargs["num_attn_candidates"] = num_attn_candidates
        kwargs["penalty_weights"] = penalty_weights
        ...
        if generation_mode_kwargs.get("opera_decoding"):
            if generation_mode != GenerationMode.BEAM_SEARCH:
                raise ValueError("OPERA decoding currently supports beam search only (num_beams > 1, do_sample=False).")
            if generation_mode_kwargs.get("key_position") is None:
                raise ValueError("OPERA decoding requires key_position.")
            generation_config.output_attentions = True
            decoding_method = getattr(type(self), "_opera_beam_search")
    ...
    def _opera_select_next_beams(
        ...
        """Select the next active beams from flattened scores for batch_size=1."""
        vocab_size = flat_scores.shape[-1] // num_beams
        topk_scores, topk_tokens = torch.topk(flat_scores, k=2 * num_beams, dim=-1, largest=True, sorted=True)
        topk_scores = topk_scores[0]
        topk_tokens = topk_tokens[0]

        next_scores: list[torch.Tensor] = []
        next_tokens: list[int] = []
        next_indices: list[int] = []

        for cand_score, cand_token in zip(topk_scores, topk_tokens):
            beam_idx = int((cand_token // vocab_size).item())
            token_id = int((cand_token % vocab_size).item())
            seq = torch.cat([input_ids[beam_idx], input_ids.new_tensor([token_id])], dim=0)

            if token_id in eos_token_ids:
                finished.append((float(cand_score.item()), seq.detach().clone()))
                continue

            next_scores.append(cand_score)
            next_tokens.append(token_id)
            next_indices.append(beam_idx)
            if len(next_scores) == num_beams:
                break

        if len(next_scores) == 0:
            for cand_score, cand_token in zip(topk_scores[:num_beams], topk_tokens[:num_beams]):
                beam_idx = int((cand_token // vocab_size).item())
                token_id = int((cand_token % vocab_size).item())
                next_scores.append(cand_score)
                next_tokens.append(token_id)
        ...
        next_scores_t = torch.stack(next_scores).to(device)
        next_tokens_t = torch.tensor(next_tokens, dtype=torch.long, device=device)
        next_indices_t = torch.tensor(next_indices, dtype=torch.long, device=device)
        return next_scores_t, next_tokens_t, next_indices_t
    ...
    def _opera_beam_search(
        ...
        **model_kwargs,
        ...
        """OPERA decoding on top of transformers v5.5 beam path (decoder-only, batch=1)."""
        del synced_gpus

        if key_position is None:
            raise ValueError("OPERA decoding requires key_position.")
        if self.config.is_encoder_decoder:
            raise NotImplementedError("OPERA decoder in this port currently supports decoder-only models.")

        num_beams = generation_config.num_beams
        num_return_sequences = generation_config.num_return_sequences
        if num_beams < 2:
            raise ValueError("OPERA requires num_beams >= 2.")
        if input_ids.shape[0] != num_beams:
            raise ValueError("OPERA currently supports batch_size=1 only.")
        if num_attn_candidates < 1:
            raise ValueError("num_attn_candidates must be >= 1.")
        ...
        if eos_token_id is None:
            eos_ids = []
        elif eos_token_id.ndim == 0:
            eos_ids = [int(eos_token_id.item())]
        else:
            eos_ids = [int(x.item()) for x in eos_token_id]

        pad_token_id = generation_config._pad_token_tensor
        pad_id = int(pad_token_id.item()) if pad_token_id is not None else 0
        ...
        model_kwargs_ori = copy.copy(model_kwargs)
        ...
        while cur_len < max_length:
            ...
            if return_dict_in_generate:
                if output_scores:
                    all_scores += (logits.clone(),)
                if output_attentions:
                    decoder_attentions += ((outputs.attentions,),)

            def _last_attn(model_outputs: Any) -> torch.Tensor:
                attn = getattr(model_outputs, "attentions", None)
                if isinstance(attn, tuple) and len(attn) > 0 and attn[-1] is not None:
                    return attn[-1]
                dec_attn = getattr(model_outputs, "decoder_attentions", None)
                if isinstance(dec_attn, tuple) and len(dec_attn) > 0 and dec_attn[-1] is not None:
                    return dec_attn[-1]
                raise RuntimeError("OPERA requires attentions, but model returned none.")

            if attn_previous is None:
                attn_previous = _last_attn(outputs).clone()
            else:
                ...
                attn_curr = _last_attn(outputs).clone().max(1, keepdim=True).values.data
                target_w = attn_previous.shape[-1]
                if attn_curr.shape[-1] != target_w:
                    if attn_curr.shape[-1] > target_w:
                        attn_curr = attn_curr[..., -target_w:]
                    else:
                        pad = torch.zeros(
                            attn_curr.shape[:-1] + (target_w - attn_curr.shape[-1],),
                            dtype=attn_curr.dtype,
                            device=attn_curr.device,
                        )
                        attn_curr = torch.cat([pad, attn_curr], dim=-1)
                attn_previous = torch.cat(
                    [attn_previous[beam_idx], attn_curr], -2
                )

            attn_previous = attn_previous.max(1, keepdim=True).values.data
            current_state["attn_previous"] = attn_previous.data.cpu()

            next_token_scores, candidate_tokens = torch.topk(logits, num_attn_candidates, dim=-1, largest=True, sorted=True)
            current_state["candidate_tokens"] = candidate_tokens.clone()

            attn_last: list[torch.Tensor] = []
            for candidate_id in range(num_attn_candidates):
                model_kwargs_tmp = copy.copy(model_kwargs)
                model_kwargs_tmp = self._update_model_kwargs_for_generation(
                    outputs, model_kwargs_tmp, is_encoder_decoder=False
                )
                candidate_token = candidate_tokens[:, candidate_id].unsqueeze(-1)
                # Qwen2.5-VL's multimodal cache path is not stable with full-sequence
                # probing here; token-wise probing keeps attention masks/positions valid.
                model_inputs_tmp = self.prepare_inputs_for_generation(candidate_token, **model_kwargs_tmp)
                model_inputs_tmp.pop("output_attentions", None)
                outputs_tmp = self(**model_inputs_tmp, return_dict=True, output_attentions=True)

                attn_output = _last_attn(outputs_tmp).clone().max(1, keepdim=True).values.data
                attn_square = torch.cat([attn_previous, torch.zeros_like(attn_previous).sum(-1, keepdim=True)], -1)
                target_w = attn_square.shape[-1]
                if attn_output.shape[-1] != target_w:
                    # Some multimodal decoder implementations return duplicated-prefix attention
                    # when candidate probing is run with cached states. Keep the most recent window.
                    if attn_output.shape[-1] > target_w:
                        attn_output = attn_output[..., -target_w:]
                    else:
                        pad = torch.zeros(
                            attn_output.shape[:-1] + (target_w - attn_output.shape[-1],),
                            dtype=attn_output.dtype,
                            device=attn_output.device,
                        )
                        attn_output = torch.cat([pad, attn_output], dim=-1)
                attn_square = torch.cat([attn_square, attn_output], -2)
                attn_last.append(attn_square)

            attn_last = torch.cat(attn_last, dim=1)
            attn_last = attn_last / attn_last.sum(-1, keepdim=True)

            attn_local = attn_last[:, :, key_position["response_start"] :, key_position["response_start"] :]
            ...
            for j in range(attn_local.shape[-1]):
                attn_local_scores[..., j] = 1e-7 * attn_local[..., j:, j].prod(-1)

            cur_response_lens = attn_local.shape[-1]
            attn_img = attn_last[:, :, -1, key_position["image_start"] : key_position["image_end"] + 1].sum(-1)
            rollback_scores = attn_local_scores.max(-1).values
            penalty_scores = -attn_img if cur_response_lens <= max(10, threshold // 2) else rollback_scores
            candidate_scores = next_token_scores - penalty_weights * penalty_scores

            rollback_locs = attn_local_scores.max(-1).indices
            ...
            current_state["candidate_token_scores"] = candidate_scores.clone()
            if len(history_states) >= history_length:
                history_states.pop(0)
            history_states.append(current_state)

            try:
                if all((x == rollback_loc).long().sum() > int(threshold) for x in rollback_loc_gathers):
                    if rollback_loc < 10:
                        raise RuntimeError("rollback too early")

                    rollback_pos = rollback_loc + 1
                    if max_rollback_time[rollback_pos] >= num_attn_candidates:
                        rollback_pos = rollback_pos - 1
                        if max_rollback_time[rollback_pos] >= num_attn_candidates:
                            raise RuntimeError("rollback maxed")
                    ...
                    candidate_scores = history_states[-2]["candidate_token_scores"]
                    ...
                    model_kwargs = copy.copy(model_kwargs_ori)
                    if "attention_mask" in model_kwargs:
                        cur_attn = model_kwargs["attention_mask"]
                        need_len = input_ids[:, :-1].shape[1]
                        if need_len > cur_attn.shape[1]:
                            pad = torch.ones(
                                (input_ids.shape[0], need_len - cur_attn.shape[1]),
                                dtype=cur_attn.dtype,
                                device=input_ids.device,
                            )
                            model_kwargs["attention_mask"] = torch.cat([cur_attn, pad], dim=1)

                    model_inputs_tmp = self.prepare_inputs_for_generation(input_ids[:, :-1], **model_kwargs)
                    model_inputs_tmp.pop("output_attentions", None)
                    outputs_tmp = self(**model_inputs_tmp, return_dict=True, output_attentions=False)
                    model_kwargs = self._update_model_kwargs_for_generation(
                        outputs_tmp, model_kwargs, is_encoder_decoder=False
                    )

                    model_inputs_tmp = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
                    model_inputs_tmp.pop("output_attentions", None)
                    outputs = self(**model_inputs_tmp, return_dict=True, output_attentions=False)
                    logits = outputs.logits[:, -1, :].to(copy=True, dtype=torch.float32, device=input_ids.device)
                    ...
                    next_token_logits = logits.new_full(logits.shape, fill_value=-999.0)
                    next_token_logits = next_token_logits - (999.0 + next_token_logits.min(-1, keepdim=True).values)
                    vocab_size = next_token_logits.shape[-1]
                    next_token_logits = next_token_logits.view(1, num_beams * vocab_size)
                    beam_idx = beam_idx.view(1, num_beams)
                    beam_next_tokens = beam_next_tokens.view(1, num_beams)
                    ...
                    next_token_logits = next_token_logits.scatter_(-1, reject_token_pos, -999.0)
                    next_token_logits = next_token_logits.view(num_beams, vocab_size)
                else:
                    raise RuntimeError("no rollback")
            except Exception:
                next_token_logits = logits.new_full(logits.shape, fill_value=-999.0)
                next_token_logits = next_token_logits.scatter(-1, candidate_tokens, candidate_scores)

            next_token_log_probs = nn.functional.log_softmax(next_token_logits, dim=-1)
            next_token_log_probs = logits_processor(input_ids, next_token_log_probs)
            next_token_log_probs = next_token_log_probs + beam_scores[:, None]

            flat_scores = next_token_log_probs.view(1, num_beams * next_token_log_probs.shape[-1])
            beam_scores, beam_next_tokens, beam_idx = self._opera_select_next_beams(
                input_ids,
                flat_scores,
                num_beams,
                eos_ids,
            ...
            if model_kwargs.get("past_key_values") is not None:
                ...
                if hasattr(self, "_reorder_cache"):
                    model_kwargs["past_key_values"] = self._reorder_cache(pkv, beam_idx)
                elif hasattr(pkv, "reorder_cache"):
                    pkv.reorder_cache(beam_idx)
                    model_kwargs["past_key_values"] = pkv
                elif hasattr(pkv, "batch_select_indices"):
                    pkv.batch_select_indices(beam_idx)
                    model_kwargs["past_key_values"] = pkv
                else:
                    raise RuntimeError("No cache reorder API found for OPERA beam decoding.")

            attn_previous = _last_attn(outputs).clone().max(1, keepdim=True).values
            cur_len += 1
            if len(finished) >= num_beams:
                break

            stop_flag = stopping_criteria(input_ids, all_scores)
            if isinstance(stop_flag, torch.Tensor):
                if bool(torch.all(stop_flag).item()):
                    break
            elif bool(stop_flag):
                break
        ...
        if len(finished) > 0:
            ...
            selected = [x[1] for x in finished[:num_return_sequences]]
        else:
            order = torch.argsort(beam_scores, descending=True)
            selected = [input_ids[int(i.item())] for i in order[:num_return_sequences]]

        max_len = max(x.shape[0] for x in selected)
        output = []
        for seq in selected:
            if seq.shape[0] < max_len:
                seq = torch.cat([seq, seq.new_full((max_len - seq.shape[0],), pad_id)])
            output.append(seq)
        sequences = torch.stack(output, dim=0)
