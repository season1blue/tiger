...
def sample(
    ...
    **model_kwargs,
    ...
    while True:
        ...
        use_cd = model_kwargs.get("input_ids_cd") is not None or model_kwargs.get("inputs_embeds_cd") is not None
        use_image_cd = model_kwargs.get("inputs_embeds_image_cd") is not None
        ...
        if use_cd:
            ...
            # print("attention mask: ", model_kwargs_cd.get("attention_mask"))
            if use_image_cd:
                if model_kwargs_image_cd is None:
                    model_kwargs_image_cd = model_kwargs.copy()
                    model_kwargs_image_cd["attention_mask"] = model_kwargs_cd["attention_mask_image_cd"]

                # print("attention mask image: ", model_kwargs_image_cd.get("attention_mask"))

                model_inputs_image_cd = self.prepare_inputs_for_generation_image_cd(input_ids, **model_kwargs_image_cd)
                outputs_image_cd = self(
                    **model_inputs_image_cd,
                    return_dict=True,
                    output_attentions=output_attentions_wo_img,
                    output_hidden_states=output_hidden_states_wo_img,
                )
                next_token_logits_image_cd = outputs_image_cd.logits[:, -1, :]
                diffs_image_cd = (1+cd_alpha)*next_token_logits - cd_alpha*next_token_logits_image_cd
                image_cd_logits = diffs_image_cd.masked_fill(next_token_logits < cutoff, -float("inf"))
            else:
                image_cd_logits = torch.zeros_like(cd_logits)
            
            cd_logits = cd_logits + image_cd_logits
        ...
        if use_image_cd:
            model_kwargs_image_cd = self._update_model_kwargs_for_generation(
                outputs_image_cd, model_kwargs_image_cd, is_encoder_decoder=self.config.is_encoder_decoder
            )
