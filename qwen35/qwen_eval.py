...
from transformers550.models.qwen3_5.modeling_qwen3_5 import Qwen3_5ForConditionalGeneration

from llava.mm_utils import get_model_name_from_path
from llava.utils import disable_torch_init
from qwen35.analysis_logger import normalize_binary_answer, trace_qwen25_sample
from memvr import apply_memvr_qwen35
...
def postprocess_qwen35_answer(text: str | None) -> str:
    if text is None:
        return ""
    ...
    marker_patterns = [
        r"(?is)\b(final answer|answer|short answer|response)\s*[:\-]\s*(.+)$",
        r"(?is)\bconclusion\s*[:\-]\s*(.+)$",
    ]
    ...
    yes_no_matches = list(re.finditer(r"\b(yes|no)\b", cleaned, flags=re.IGNORECASE))
    if yes_no_matches:
        return yes_no_matches[-1].group(1).lower()

    if "true" in lowered or "false" in lowered:
        tf_matches = list(re.finditer(r"\b(true|false)\b", cleaned, flags=re.IGNORECASE))
        if tf_matches:
...
def run_qa_eval(args, model_name, processor, model):
    ...
    with open(answers_file, "w") as ans_file:
        for line in tqdm(questions, total=len(questions), desc="Qwen eval", ncols=100):
            ...
            with torch.inference_mode():
                ...
                inputs = apply_qwen_chat_template(processor, messages)
                ...
                if args.analysis_log_dir:
                    ...
                    analysis_metadata = {
                        ...
                        "prompt": prompt,
                        "raw_generated_answer": raw_output,
    ...
    try:
        ...
        print(f"[MemVR Stats Qwen3.5] warning: failed to write stats_file={stats_file}: {exc}")
        stats_file = "<write_failed>"

    print(
        f"[MemVR Stats Qwen3.5] total_samples={total_samples} "
...
def build_parser():
    ...
    parser.add_argument("--model-path", type=str, default="/data/ssz/llms/Qwen3.5-VL-7B-Instruct")
